...
function App() {
  ...
  const analyzeVideo = async (videoId: string) => {
    try {
      const res = await fetch(`${API_BASE}/analyze-video/${videoId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: prompt || "Make it watchable",
          blacklist: blacklist
            ? blacklist.split(",").map((w) => w.trim())
            : [],
        }),
      });
      const data = await res.json();
  ...
  const editVideos = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/edit-video`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          video_ids: videoIds,
      ...
      if (data.output_video) {
        const videoUrl = `${API_BASE}/${data.output_video}`;
        setOutputVideoUrl(videoUrl);
        alert("Videos edited and spliced successfully!");
      } else {
        alert("No output video generated.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-700 text-white">
      ...
      <main className="flex-1 flex flex-row bg-gray-700 p-4">
        {/* Sidebar for prompt & blacklist */}
        <section className="w-1/6 min-w-[180px] max-w-xs bg-gray-700 flex flex-col p-2 border-r border-gray-700">
          <textarea
            className="flex-1 min-w-0 rounded-xl bg-gray-800 text-white px-5 py-3 focus:outline-none resize-none mb-2 min-h-[80px]"
            placeholder="Enter prompt..."
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
          />
          <textarea
            className="flex-1 min-w-0 rounded-xl bg-gray-800 text-white px-5 py-3 focus:outline-none resize-none mb-2 min-h-[60px]"
            placeholder="Blacklist words (comma-separated)"
            value={blacklist}
              ...
              {videos.map((v) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => analyzeVideo(v.video_id)}
                  >
                    Analyze
                  </button>
                </div>
